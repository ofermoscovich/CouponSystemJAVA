...
public class AdminFacade extends Clients implements CouponClientFacade { 
	...
	public void removeCompany(long compId) throws CouponException {
		if (compId > 0) {
			if (companyDAO.getCompany(compId).getId() > 0) {
				companyDAO.removeCompany(compId);
			} else {
				throw new CouponException("STOP! Company Not Exist! Remove Company is Canceled!"); 
			}
		} else {
	...
	public void removeCustomer(long custId) throws CouponException {
		if (custId > 0) {
			if (customerDAO.getCustomer(custId).getId() > 0) {
				customerDAO.removeCustomer(custId);
			} else {
				throw new CouponException("STOP! Customer Not Exist! Remove Customer is Canceled!"); 
			}
		} else {
			throw new CouponException("STOP! No Customer was Chosen! Remove Customer is Canceled!");
		}	
	}
